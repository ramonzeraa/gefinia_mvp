...
getCategoryName(categoryId) {
    ...
    async updateGain() {
      try {
        await axios.put(`http://localhost:5000/api/gains/${this.editingGain.id}`, this.editingGain);
    ...
    async deleteGain(gainId) {
      ...
      if (confirm('Tem certeza que deseja deletar este ganho?')) {
        try {
          await axios.delete(`http://localhost:5000/api/gains/${gainId}`);
    ...
    async updateExpense() {
      try {
        await axios.put(`http://localhost:5000/api/expenses/${this.editingExpense.id}`, this.editingExpense);
        this.editingExpense = null;
        this.fetchData();
      } catch (error) {
  ...
  // watch: {
  //   editingGain(newValue) {
  //     if (newValue) this.updateGain();
  //   },
  //   editingExpense(newValue) {
  //     if (newValue) this.updateExpense();
  //   }
  // },
  mounted() {
    this.fetchData();
